...
export function CreateStreamModal({ isOpen, onClose }: CreateStreamModalProps) {
  ...
  const handleCreateStream = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://api.streamhivex.icu/api/streams/create', {
        method: 'POST',
        headers: {
...
export function ContentSuggestionSubModal({ onSelectContent, onClose }: ContentSuggestionSubModalProps) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchTerm.trim() !== '') {
        setLoading(true);
        fetch(`https://api.streamhivex.icu/api/contents?termo=${encodeURIComponent(searchTerm)}`)
